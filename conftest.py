...
# from selenium.webdriver.opera.options import Options as OpOptions
...
#Hasn't been tested with opera yet!
    elif browser == 'headlessopera':
        op_options = OpOptions()
        op_options.add_argument("--disable-gpu")
        op_options.add_argument("--no-sandbox")
        op_options.add_argument("--headless")
        driver = webdriver.Opera(options=op_options)
...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == 'call':
        ...
        if (report.skipped and xfail) or (report.failed and not xfail):
            ...
            if is_frontend_test:
                results_dir = os.environ.get('REPORTS_DIR')
                if not results_dir:
                    raise Exception("Environment variable 'REPORTS_DIR' must be set.")
...
#                     raise Exception("Environment variable 'REPORTS_DIR' must be set.")
        ...
        #         # extra.append(pytest_html.extras.html('<div style="background:blue;">Additional HTML</div>'))
